...
namespace GeneticAlgorithms
{
	...
	{
		public class RankSelectionMethod:ISelectionMethod
		{
			private static Random random = new Random();
			
			public RankSelectionMethod()
			{
			}
			...
			public Chromosomes.IChromosome[] PerformSelection (Populations.IPopulation population, int size)
			{
				...
				//Assigning ranks as fitness and Calculating the sum of the fitnesses.
				double sum = 0.0;
				for (int i = 0; i < sz; ++i){
					f[i]=sz-i+1;
					sum += f[i];
				...
				for (int i = 0; i < size; ++i)
					ret[i] = Find(individuals, f, random.NextDouble()).Clone();
